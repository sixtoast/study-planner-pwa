...
function isHardSubject(subject: string): boolean {
  return getDifficultyWeight(subject) >= 4;
}
...
export function generateDailyTimetable(options?: {
  ...
  for (let i = 0; i < totalDays; i++) {
    ...
    // Rest day every 5th day (more frequent recovery)
    const isScheduledBreak = i > 0 && i % 5 === 0;
    ...
    if (isScheduledBreak && !hasExamTomorrow) {
      plans.push({
        ...
        isBreak: true,
        breakReason:
          "Rest day – complete rest or very light review of one weak topic only. Recover so you can study effectively tomorrow.",
        tasks: [],
        totalMinutes: 0,
      });
      continue;
    ...
    let hardSubjectsToday = 0; // max 1 hard subject per day
    ...
    for (const exam of sorted) {
      ...
      const hard = isHardSubject(exam.subject);

      // RULE: never put more than one hard subject on the same day
      if (hard && hardSubjectsToday >= 1) continue;

      // Easy subjects don't need many early sessions
    ...
    if (tasks.length === 0) {
      plans.push({
        ...
        dayLabel,
        isBreak: true,
        breakReason: "Rest day – complete rest or very light review only",
        tasks: [],
        totalMinutes: 0,
      });
    } else {
